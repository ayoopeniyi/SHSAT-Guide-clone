...
export default function Consultation_callPage() {
	...
	const [selectedEvent, setSelectedEvent] = useState<number | null>(3065973);
		...
		api
			...
			.catch((err: AxiosError<any>) => {
				...
				setError(
					...
					"Failed to load event types. Check backend URL and CORS."
				);
			})
	...
	const fetchSlots = async () => {
		...
		try {
			...
			if (normalized.length === 0) {
				...
				setActiveStep(2); // Move to session details step
			}
		} catch (err) {
	...
	const handleBooking = async (slotISO?: string) => {
		...
		if (!description || !description.trim()) {
			setError("Please enter a session description.");
			return;
		}
				...
				<div className="bg-[#377e9a] text-white p-8 relative">
					...
					<div className="text-center pt-4">
						<h1 className="text-3xl md:text-4xl font-bold mb-3">
							SHSAT Consultation Booking
						</h1>
						<p className="text-blue-100 opacity-90 max-w-2xl mx-auto">
							Schedule your personalized SHSAT preparation session with our
							experts. Get tailored advice to help you succeed on the test.
						</p>
					</div>
				</div>
				...
				<div className="p-6 md:p-8">
						...
						<div className="mb-8">
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 mb-5">
								...
								<div>
									...
									<div className="relative">
										...
										<input
											type="date"
											className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#377e9a] focus:border-transparent transition-all duration-200"
											value={fromDate}
											min={today}
							...
							<div className="mb-6">
								<button
									...
									className="w-full bg-[#377e9a] hover:bg-[#2c5d6b] disabled:bg-gray-400 text-white font-medium py-3 px-6 rounded-lg transition-all duration-200 flex items-center justify-center"
								>
									{loadingSlots ? (
										<>
											<svg
												...
												{slots
													...
													.map((slot, i) => (
														<button
															...
															disabled={submitting}
															className="p-3 border-2 border-gray-200 rounded-lg text-center transition-all duration-200 hover:border-[#377e9a] hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed"
					...
					{activeStep >= 2 && selectedSlot && (
						<div className="mb-8 p-6 bg-gray-50 rounded-xl border border-gray-200">
							<h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center">
								<div className="bg-blue-100 p-2 rounded-lg mr-3">
									<BookOpen className="w-5 h-5 text-[#377e9a]" />
								</div>
								Session Details
							</h2>
							...
							<div className="mb-5 p-4 bg-white rounded-lg border border-gray-200">
								<div className="flex justify-between items-center mb-3">
									...
									<span className="font-semibold text-blue-700">
										{selectedSlot.date} at {selectedSlot.user_time} ({timezone})
									</span>
								</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
								<div>
									...
									<input
										type="text"
										className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#377e9a] focus:border-transparent transition-all duration-200"
										value={title}
										onChange={(e) => setTitle(e.target.value)}
										placeholder="e.g., Math Section Review"
									/>
								</div>
							</div>
							...
							<div className="mb-5">
								...
								<textarea
									...
									rows={3}
									placeholder="Brief description of what you'd like to discuss (e.g., 'Need help with geometry problems and time management')"
								/>
							</div>
							...
							{activeStep === 2 && (
								<div className="mt-6 flex justify-between">
									...
									<button
										...
										className="bg-[#377e9a] hover:bg-[#2c5d6b] disabled:bg-gray-400 text-white font-medium py-2.5 px-6 rounded-lg transition-all duration-200 flex items-center"
									>
